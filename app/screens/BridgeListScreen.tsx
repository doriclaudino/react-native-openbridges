...
class BridgeListScreen extends Component<Props, State> {
    state: State = {
        bannerVisible: false,
    ...
    static navigationOptions = ({ navigation }: NavigationScreenProps<NavigationParams>) => {
        const isSearchBarVisible = navigation.getParam('isSearchBarVisible', undefined)
        const onSearchBarChangeText = navigation.getParam('onSearchBarChangeText', undefined)
        const onSearchClick = navigation.getParam('onSearchClick', undefined)
        const searchBarValue = navigation.getParam('searchBarValue', undefined)

        const isSliderLocationVisible = navigation.getParam('isSliderLocationVisible', undefined)
        const sliderLocationOptions = navigation.getParam('sliderLocationOptions', undefined)

        const onSliderLocationCloseClick = navigation.getParam('onSliderLocationCloseClick', undefined)
        const onDistanceChange = navigation.getParam('onDistanceChange', undefined)
        const selectedDistance = navigation.getParam('selectedDistance', undefined)

        const currentUserLocation = navigation.getParam('currentUserLocation', undefined)
        const onFilterLocationClick = navigation.getParam('onFilterLocationClick', undefined)
            ...
            return ({
                header: (
                    <SearchAppbar
                        ...
                        onChangeText={(text: string) => { onSearchBarChangeText(text); navigation.setParams({ searchBarValue: text }) }}
                        value={searchBarValue}
                        onIconPress={onSearchClick}
                    />
                )
            })
        } else if (isSliderLocationVisible) {
            let minimumValue, maximumValue, step = undefined;
        ...
        return ({
            header: (
                <Appbar.Header>
                    ...
                    {currentUserLocation && < Appbar.Action disabled={!onFilterLocationClick} icon={() => <Entypo name="ruler" size={24} color="white" />} onPress={() => { onFilterLocationClick() }} />}
                    < Appbar.Action disabled={!onRequestLocationClick} icon={locationIcon} onPress={() => { onRequestLocationClick() }} />
                </Appbar.Header>
            )
        })
    };
    ...
    componentDidMount = () => {
        ...
        this.setState({ bannerTimeout: bannerTimeout })
    }
    ...
    _watchUserPosition = async () => {
        try {
            ...
            if (granted === PermissionsAndroid.RESULTS.GRANTED) {
                ...
                const listener = navigator.geolocation.watchPosition(this._sucessPosition, this._onWatchUserLocationError, this.state.gpsOptions);
